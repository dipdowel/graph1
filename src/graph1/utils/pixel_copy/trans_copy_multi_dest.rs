...
use crate::graph1::primitives::primitives::{Point, RectArea};

/// Transparency-aware copy
/// - Copies non-transparent pixels from the source memory buf to multiple destinations.
/// - `src_buf_view` -- source memory buffer
/// - `dst_buf_view` -- destination memory buffer
/// - `src_area` -- A rectangular area that will be copied
/// - `dst_start` -- A list of top-left points of the destination areas.
...
pub fn trans_copy_multi_dest(
    ...
    for single_start in dst_start {
        let mut dst_x = single_start.x;
        let mut dst_y = single_start.y;
