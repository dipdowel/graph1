...
#[inline(always)]
fn parse_lines(lines: &Vec<u32>) -> Vec<LineMeta> {
...
#[inline(always)]
fn draw_band_lines(band_lines: &[&LineMeta], band_slice: &mut [u32], y_start: u32, width: u32, height: u32) {
    for line in band_lines {
        if line.y >= height { continue; }
        let max_x = width - 1;
        let x_start = line.x_start.clamp(0, max_x);
        let x_end = line.x_end.clamp(0, max_x);
...
pub fn horizontal_lines_x4_threaded(buf: &mut [u32], buf_dimensions: &Dimensions2d, lines: &Vec<u32>, num_threads: usize) {
    ...
    // 1. Parse and sort
    let mut parsed_lines = parse_lines(lines);
