...
/// Every 0u32 value gets converted to `0`
...
/// to a Vec<u32> RGBA representation, where:
/// - bit `0` translates to `0x00_00_00_ff`
/// - bit `1` translates to `0x00_ff_ff_ff`
pub fn one_bit_image_to_rgba(bit_array: &Vec<u8>) -> Vec<u32> {
    ...
    for byte in bit_array {
        for bit_index in 0..8 {
            ...
            // Translate bit 0 to 0x000000ff and bit 1 to 0xffffffff
            if bit == 0 {
                data.push(0x00_00_00_ff);
            } else {
                data.push(0xff_ff_ff_ff);
            }
        }
    }
    data
...
mod tests {
    ...
    fn test_convert_to_bit_array_basic() {
        ...
        let bit_array = rgba_to_one_bit_image(&data);
    ...
    fn test_convert_from_bit_array_basic() {
        ...
        let data = one_bit_image_to_rgba(&bit_array);
        assert_eq!(
            data,
            vec![
                0x000000ff, 0xffffffff, 0x000000ff, 0xffffffff, 0x000000ff, 0x000000ff, 0x000000ff,
                0x000000ff
            ]
        );
    }
    ...
    fn test_convert_from_bit_array_partial_byte() {
        ...
        assert_eq!(
            data,
            vec![
                0xffffffff, 0x000000ff, 0xffffffff, 0x000000ff, 0xffffffff, 0x000000ff, 0x000000ff,
                0x000000ff
            ]
        );
    }
    ...
    fn test_convert_from_bit_array_mixed() {
        ...
        assert_eq!(
            data,
            vec![
                0x000000ff, 0xffffffff, 0xffffffff, 0x000000ff, 0xffffffff, 0x000000ff, 0x000000ff,
                0x000000ff, 0xffffffff, 0x000000ff, 0x000000ff, 0x000000ff, 0x000000ff, 0x000000ff,
