...
mod tests {
    ...
    #[test]
    fn test_from_u32() {
        let pixel = from_rgb::of_u32(TEST_R as u32, TEST_G as u32, TEST_B as u32, TEST_A as u32);
        assert_eq!(pixel, TEST_PIXEL);

        // Test masking behavior - values > 255 should be masked
        let pixel = from_rgb::of_u32(0x1FF, 0x180, 0x140, 0x120);
        assert_eq!(pixel, TEST_PIXEL);
    }

    #[test]
    fn test_from_u8() {
        let pixel = from_rgb::of_u8(TEST_R, TEST_G, TEST_B, TEST_A);
        assert_eq!(pixel, TEST_PIXEL);

        let pixel = from_rgb::of_u8(0, 0, 0, 0);
        assert_eq!(pixel, BLACK_TRANSPARENT);

        let pixel = from_rgb::of_u8(255, 255, 255, 255);
        assert_eq!(pixel, WHITE_OPAQUE);
    }

    #[test]
    fn test_from_u16() {
        let pixel = from_rgb::of_u16(TEST_R as u16, TEST_G as u16, TEST_B as u16, TEST_A as u16);
        assert_eq!(pixel, TEST_PIXEL);

        // Test masking behavior - values > 255 should be masked
        let pixel = from_rgb::of_u16(0x1FF, 0x180, 0x140, 0x120);
        assert_eq!(pixel, TEST_PIXEL);
    }

    #[test]
    fn test_from_f32() {
        let pixel = from_rgb::of_f32(TEST_R as f32, TEST_G as f32, TEST_B as f32, TEST_A as f32);
        assert_eq!(pixel, TEST_PIXEL);

        let pixel = from_rgb::of_f32(0.0, 0.0, 0.0, 0.0);
        assert_eq!(pixel, BLACK_TRANSPARENT);

        let pixel = from_rgb::of_f32(255.0, 255.0, 255.0, 255.0);
        assert_eq!(pixel, WHITE_OPAQUE);
    }

    #[test]
    fn test_from_f64() {
        let pixel = from_rgb::of_f64(TEST_R as f64, TEST_G as f64, TEST_B as f64, TEST_A as f64);
        assert_eq!(pixel, TEST_PIXEL);

        let pixel = from_rgb::of_f64(0.0, 0.0, 0.0, 0.0);
        assert_eq!(pixel, BLACK_TRANSPARENT);

        let pixel = from_rgb::of_f64(255.0, 255.0, 255.0, 255.0);
        assert_eq!(pixel, WHITE_OPAQUE);
    }
    ...
    fn test_from_f32_normalized() {
        ...
        let pixel = from_rgb::of_f32_normalized(1.0, 0.5, 0.25, 0.125);
        ...
        let pixel = from_rgb::of_f32_normalized(0.0, 0.0, 0.0, 0.0);
        assert_eq!(pixel, BLACK_TRANSPARENT);

        let pixel = from_rgb::of_f32_normalized(1.0, 1.0, 1.0, 1.0);
        assert_eq!(pixel, WHITE_OPAQUE);

        // Test clamping
        let pixel = from_rgb::of_f32_normalized(2.0, -1.0, 0.5, 1.5);
        let (r, g, b, a) = to_rgb::as_u8(pixel);
    ...
    fn test_from_f64_normalized() {
        // Use exact normalized values
        let pixel = from_rgb::of_f64_normalized(1.0, 0.5, 0.25, 0.125);
        let (r, g, b, a) = to_rgb::as_u8(pixel);
        assert_eq!(r, 255); // 1.0 * 255 = 255
        assert_eq!(g, 127); // 0.5 * 255 = 127.5 -> 127
        assert_eq!(b, 63);  // 0.25 * 255 = 63.75 -> 63
        ...
        let pixel = from_rgb::of_f64_normalized(0.0, 0.0, 0.0, 0.0);
        assert_eq!(pixel, BLACK_TRANSPARENT);

        let pixel = from_rgb::of_f64_normalized(1.0, 1.0, 1.0, 1.0);
        assert_eq!(pixel, WHITE_OPAQUE);

        // Test clamping
        let pixel = from_rgb::of_f64_normalized(2.0, -1.0, 0.5, 1.5);
        let (r, g, b, a) = to_rgb::as_u8(pixel);
        assert_eq!(r, 255); // clamped from 2.0
        assert_eq!(g, 0);   // clamped from -1.0
        assert_eq!(b, 127); // 0.5 * 255
    ...
    fn test_roundtrip_f32_normalized() {
        ...
        let pixel = from_rgb::of_u8(100, 150, 200, 250);
        let (r, g, b, a) = to_rgb::as_f32_normalized(pixel);
        let pixel2 = from_rgb::of_f32_normalized(r, g, b, a);
    ...
    fn test_roundtrip_f64_normalized() {
        ...
        let pixel2 = from_rgb::of_f64_normalized(r, g, b, a);
        assert_eq!(pixel, pixel2);
    }
    ...
    fn test_pixel_format() {
        ...
        let pixel = from_rgb::of_u8(0xAA, 0xBB, 0xCC, 0xDD);
    ...
    fn test_all_channels_independent() {
        ...
        let pixel_r = from_rgb::of_u8(255, 0, 0, 0);
        assert_eq!(pixel_r, 0xFF00_0000);

        let pixel_g = from_rgb::of_u8(0, 255, 0, 0);
        assert_eq!(pixel_g, 0x00FF_0000);

        let pixel_b = from_rgb::of_u8(0, 0, 255, 0);
        assert_eq!(pixel_b, 0x0000_FF00);

        let pixel_a = from_rgb::of_u8(0, 0, 0, 255);
