...
pub fn star(buf_view: &mut [u32],props: &StarProperties) {
    ...
    if let Some(first_vertex) = vertices.first() {
        ...
        for vertex in vertices.iter().skip(1) {
            between_two_points(buf_view, &Pixel{x:previous_vertex.x, y:previous_vertex.y, color:props.center.color}, vertex);
            previous_vertex = vertex;
        }
        // Connect the last vertex to the first to complete the polygon
        between_two_points(buf_view, &Pixel{x:previous_vertex.x, y:previous_vertex.y, color:props.center.color}, first_vertex);
