...
// FIXME: remove the old implementation!
...
fn render_points<UserData, PointType: Numeric>(
    ctx: &mut GraphContext<UserData>,
    points: &[Point<PointType>],
) {
    for p in points {
        let point_color = ctx.bezier.start_end_points_color;

        if point_color.is_some() {
            render_point_color(
                ctx,
                &p.convert(),
                point_color.unwrap_or(ctx.win.foreground_color),
            );
        } else {
            render_point_inverted(ctx, &p.convert());
        }
    }
}
...
fn draw_controls<UserData, PointType: Numeric>(
    ctx: &mut GraphContext<UserData>,
    ...
    let normalized: Vec<Point> = points
        ...
        .map(|p| {
            Point::new(
                i32::max(p.x.to_i32(), 0) as u32,
                i32::max(p.y.to_i32(), 0) as u32,
            )
        })
        .collect();
        ...
        for pair in normalized.windows(2).step_by(2) {
            if let [p1, p2] = pair {
                line::between_two_points(
                    ...
                    &p1.convert(),
                    &p2.convert(),
                    Some(ctx.bezier.control_color.unwrap_or(ctx.win.foreground_color)),
                );
            }
        }
    }
    render_points(ctx, &normalized);
}
...
/// TODO: Write proper documentation!!!!
/// TODO: Write proper documentation!!!!
pub fn bezier<UserData, PointType: Numeric>(
    ctx: &mut GraphContext<UserData>,
    segments: &[BezierSegment<PointType>],
    resolution_delta: f32,
) {
